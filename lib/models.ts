...
// Structured exercise data from AI analysis
...
export interface WorkoutLog {
	...
	date: string; // YYYY-MM-DD
	// AI-generated fields
	aiSessionType?: "strength" | "cardio" | "mixed" | "other";
	aiExercises?: WorkoutExercise[];
	aiTotalVolume?: number;
	aiMuscleGroups?: string[];
	aiNotes?: string;
	aiMetadataId?: string;
}
...
export interface WorkoutLogDbRow {
	...
	ai_session_type: string | null;
	ai_exercises: WorkoutExercise[] | null;
	ai_total_volume: number | null;
...
export function transformWorkoutLogFromDb(row: WorkoutLogDbRow): WorkoutLog {
	return {
		...
		// AI fields
		aiSessionType:
			(row.ai_session_type as WorkoutLog["aiSessionType"]) ?? undefined,
		aiExercises: row.ai_exercises ?? undefined,
		aiTotalVolume: row.ai_total_volume ?? undefined,
		aiMuscleGroups: row.ai_muscle_groups ?? undefined,
...
export function transformWorkoutLogToDb(
	...
	return {
		...
		log_date: log.date,
		// AI fields
		ai_session_type: log.aiSessionType ?? null,
		ai_exercises: log.aiExercises ?? null,
		ai_total_volume: log.aiTotalVolume ?? null,
		ai_muscle_groups: log.aiMuscleGroups ?? null,
		ai_notes: log.aiNotes ?? null,
		ai_metadata_id: log.aiMetadataId ?? null,
	};
}
...
// Transform database row to application type
export function transformJournalEntryFromDb(
	row: JournalEntryDbRow,
): JournalEntry {
	return {
		id: row.id,
		content: row.content ?? undefined,
